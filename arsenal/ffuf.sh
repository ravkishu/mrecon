...
ffuf -t 80 -c -sf -fc '404,429,501,502,503' -H "X-Forwarded-For: 127.0.0.1" -H "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:72.0) Gecko/20100101 Firefox/72.0" -D -e '.asp,.aspx,.backup,.bak,.conf,.config,.db,.gz,.gz.tar,.htaccess,.htm,.html,.js,.json,.jsp,.log,.old,.php,.py,.shtml,.sql,.swf,.swp,.tar.gz,.txt,.xml,.zip' -of csv -o $dir/$1_ffuf_raw.tmp -u "$ffufdomain/FUZZ" -w ~/tools/dirsearch/db/dicc.txt -mode clusterbomb -v -r true -recursion -recursion-depth 2
